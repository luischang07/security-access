<div class="max-w-7xl mx-auto">
    ...
    <section>
        ...
        <div class="space-y-3">
            ...
            @if($pedidosCollection && count($pedidosCollection) > 0)
                @foreach($pedidosCollection as $pedido)
                    @php
                    ...
                    <div class="rounded-lg border border-border-light dark:border-border-dark shadow-sm bg-card-light dark:bg-card-dark p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div class="flex-1 w-full">
                            <p class="text-sm text-neutral-text dark:text-neutral-text-dark">
                                {{ __('patient.orders.order_id') }}: {{ $pedido->getFolio() ?? '-' }}
                            </p>
                            <p class="text-base font-bold text-body-text dark:text-body-text-dark">
                                ...
                                @endphp
                                {{ $nombres->count() ? $nombres->join(', ') : __('patient.orders.no_items') }}
                            </p>
                            <p class="text-sm text-neutral-text dark:text-neutral-text-dark">
                                @php
                        ...
                        <div class="flex items-center justify-between w-full sm:w-auto gap-4">
                            ...
                            <a href="{{ route('patient.orders.show', $pedido->getFolio()) }}" class="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-9 px-4 bg-background-light dark:bg-background-dark text-body-text dark:text-body-text-dark text-sm font-medium leading-normal hover:bg-border-light dark:hover:bg-border-dark transition">
            ...
            @else
                <div class="rounded-lg border border-border-light dark:border-border-dark shadow-sm bg-card-light dark:bg-card-dark p-6 text-center">
                    <p class="text-neutral-text dark:text-neutral-text-dark">{{ __('patient.orders.no_orders') ?? 'No hay pedidos para mostrar.' }}</p>
                </div>
            @endif
        </div>
    </section>
