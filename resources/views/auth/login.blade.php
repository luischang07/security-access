    ...
    <div class="container">
      ...
      @if (session('show_session_reset'))
        <div class="session-reset-section">
          ...
          <form method="POST" action="{{ route('session.reset.send') }}" style="margin: 15px 0;" id="session-reset-form">
            @csrf
            <input type="hidden" name="email" value="{{ old('correo') }}">
            <button type="submit" class="reset-session-btn">
              Delete Active Session
            </button>
          </form>

          <p><small>We'll send a confirmation link to <span>{{ old('correo') }}</span></small>
          </p>
        </div>
      @endif
