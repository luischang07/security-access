...
@section('title', __('prescription.pharmacy_map.title'))

@push('styles')
  <!-- Leaflet CSS -->
  <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"
    integrity="sha256-p4NxAoJBhIIN+hmNHrzRCf9tD/miZyoHS5obTRR9BMY=" crossorigin="" />
  @vite(['resources/css/pharmacy-map.css'])
@endpush
...
@push('scripts')
  ...
  @vite(['resources/js/pharmacy-map.js'])
@endpush

@section('content')
  {{-- Use unified topbar with map navigation links --}}
  @include('components.topbar', ['type' => 'patient', 'user' => auth()->user(), 'showMapLinks' => true])
    ...
    <aside
      ...
      <div class="flex gap-2 overflow-x-auto no-scrollbar pb-2">
        ...
        <button id="filterOpen"
          ...
          <span class="text-sm font-medium">{{ __('prescription.pharmacy_map.filters.open_now') }}</span>
        </button>
        <button id="filter24h"
          class="flex h-9 shrink-0 items-center gap-2 rounded-lg bg-background-light dark:bg-background-dark px-3 hover:bg-border-light dark:hover:bg-border-dark transition-colors text-body-text dark:text-body-text-dark">
          <span class="material-symbols-outlined text-lg">history</span>
          <span class="text-sm font-medium">{{ __('prescription.pharmacy_map.filters.24_hours') }}</span>
        </button>
      </div>
        ...
        <div
          ...
          <div class="pt-2">
            <button
              ...
              {{ __('prescription.pharmacy_map.pharmacy_card.select') }}
            </button>
          </div>
        </div>
      </div>
